...
# 현재 경로에 있는  Dockerfile에 작성된 내용을 기준으로

docker build -t 이미지이름 .

#어떤 특정 경로에 있는 Dockerfile or 특정 Dockerfile 로 빌드하는 경우 -f 옵션
docker build -t 이미지이름 -f ㅡㅛ애찯ㄱ랴ㅣㄷ .
docker build -t 이미지이름 
...
#추가된 파일까지 포함하려면 이미지를 다운로드
docker build -t df2 .
docker run -d --name df2 df2

#빌드, 컨테이너실행, 내부명령어 bash
cd : 폴더이동
ls : 파일목록 확인
...
ENTRYPOINT : 컨테이너 시작했을때 실행할 명령어
ENTRYPOINT ["/bin/bash","-c","sleep second 500"]
...
#ex)
docker run -d -p 5900:5900 images
docker run -d -p 9000:9000 images
docker run -d -p 8000:8000 images
docker run -d -p images
    #호스트임시포트:컨테이너9000

ENV 환경변수 세팅

#명령어기준
docker run -d -e MYSQL_ROOT_PASSWORD=rootpw123 \
...
#dockerfile06 폴더 기준
docker build -t df6 .
docker run -d -p 3306:3306 --name dt6 dt6
